...
class TravelCardListState extends State<TravelCardList> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    _cardHeight = (size.height * .48).clamp(300, 400);
  ...
  bool _handleScrollNotifications(Notification notification) {
    ...
    if (notification is ScrollUpdateNotification) {
      if (_isScrolling) {
        var dx = notification.metrics.pixels - _prevScrollX;
  ...
  //If the user has released a pointer, and is currently scrolling, we'll assume their done scrolling and tween our offset to zero.
